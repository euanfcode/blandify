...
Every sentence and clause in the rewrite must be traceable to something stated in the source or very obviously implied by it.
...
- new future actions
- new claims about what the event demonstrates, highlights, reveals or means
...
Do NOT turn ordinary actions into "strategies", "frameworks", "journeys", "opportunities", "case studies", "content events", "engagement loops", "touchpoints", "ecosystems" or similar unless the source itself supports that language.
...
A good contrast may pompously reframe an explicit reason or an obvious implication already contained in the source.

Do not force a contrast if it requires inventing information, repeating the same noun phrase pointlessly, or exceeding the word limit.
...
Blandified:
"I cried in the car for approximately ten minutes before subsequently going into the meeting—it wasn't simply a brief moment of crying. It was a ten-minute period of weeping before the meeting."

Source:
"On Tuesday I bought seventeen peaches because the woman in the market looked disappointed."

Blandified:
"On Tuesday, I strategically acquired seventeen peaches in response to the market vendor's evident emotional disposition. This isn't casual shopping. It's intentional empathy-driven procurement."

Source:
"I accidentally emailed my landlord a photo of my lunch."

Blandified:
"I inadvertently emailed my landlord a photographic image of my lunch—an unintended instance of lunch-related image sharing."
...
RETRY REQUIREMENT:
Your previous attempt was rejected for exceeding the ${maximumWordCount}-word limit.
Rewrite the ORIGINAL source again from scratch.
Be shorter.
Do not add any sentence after the source event has been fully rewritten.
The final answer must be ${maximumWordCount} words or fewer.`;
      ...
      blandifiedText = cleanOutput(result.output_text || "");

      if (!blandifiedText) {
        continue;
      }

      if (countWords(blandifiedText) <= maximumWordCount) {
      ...
      console.warn(
        `Blandify attempt ${attempt} exceeded limit: ` +
        `${countWords(blandifiedText)} words / ${maximumWordCount} allowed.`
      );
    }
